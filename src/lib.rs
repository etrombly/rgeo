...
lazy_static! {
    static ref GEO: KdTree<f32, Record, [f32; 3]> = {
...
pub fn dot<T: Float + FromPrimitive>(left: &[T], right: &[T]) -> T  {
...
pub fn distance<T: Float + FromPrimitive>(left: &[T], right: &[T]) -> T {
...
pub fn search<'a>(lat: f32, lon: f32) -> Option<(f32, &'a record::Record)> {
    ...
    match GEO.nearest(&[nvec.x, nvec.y, nvec.z], 1, &distance) {
        Ok(x) => {
            if x.is_empty() {
                None
            } else {
